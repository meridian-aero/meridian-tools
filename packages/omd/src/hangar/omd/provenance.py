...
def provenance_dag_html(
    ...
    Uses a simple embedded Cytoscape.js visualization.
    ...
    for entity in dag["entities"]:
        nodes.append({
            "data": {
                "id": entity["entity_id"],
                "label": f"{entity.get('entity_type', '?')}\n{entity['entity_id']}",
                "type": "entity",
                "entity_type": entity.get("entity_type", "unknown"),
            }
        })
    ...
    for activity in dag["activities"]:
        nodes.append({
            "data": {
                "id": activity["activity_id"],
                "label": f"{activity.get('activity_type', '?').upper()}\n{activity.get('agent', '?')}",
                "type": "activity",
            }
        })
...
<head>
    ...
    <style>
        ...
        h1 {{ position: absolute; top: 10px; left: 10px; z-index: 1;
            ...
            style: [
                ...
                {{ selector: 'node[type="activity"]',
                   style: {{ 'background-color': '#e6a23c', 'shape': 'diamond',
                            ...
                            'font-size': '10px', 'width': 30, 'height': 30 }} }},
                {{ selector: 'edge',
