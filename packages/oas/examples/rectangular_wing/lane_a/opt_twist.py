"""Lane A: Twist optimization using raw OpenAeroStruct.

Minimises inviscid CD by varying spanwise twist (3 B-spline CPs),
subject to CL=0.5.  Expected result: elliptical lift distribution.

Migrated from: upstream/OpenAeroStruct/oas_mcp/demonstrations/rectangular_wing/lane_a/opt_twist.py
"""
...
from shared import MESH, SURFACE_AERO, FLIGHT_AERO, OPT_TWIST
...
def run() -> dict:
    ...
    prob.model.add_design_var(
        "wing.twist_cp",
        lower=OPT_TWIST["twist_cp_lower"],
        upper=OPT_TWIST["twist_cp_upper"],
    )
    prob.model.add_constraint(f"{point_name}.wing_perf.CL", equals=OPT_TWIST["cl_target"])
    prob.model.add_objective(f"{point_name}.wing_perf.CD", scaler=OPT_TWIST["scaler"])
    ...
    twist_cp = prob["wing.twist_cp"].flatten().tolist()[::-1]
